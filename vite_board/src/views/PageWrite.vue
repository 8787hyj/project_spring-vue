<template>
  ...
  <span v-for="(item, index) in fileLists" :key="index">
    {{ item.fileName }}
    <el-button v-show="item.fileName" type="primary" @click="del(fileLists, item.fileName)">삭제</el-button>
    <p></p>
  </span>
  <hr />
...
// 삭제 (list, name)
const del = (index, fileName) => {
  return ElMessageBox.confirm(`Cancel the transfert of ${fileName}?`).then(
    () => {
      console.log(fileLists.value);
      fileLists.value = fileLists.value.filter((element) => element.fileName !== fileName);
      return fileLists.value;
    },
    () => {
      console.log('bbb');
      return false;
